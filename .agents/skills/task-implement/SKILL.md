...
- **Enforce every business rule server-side** — validation of enum values, references, and invariants happens in the backend; client-side validation alone is never sufficient (AGENTS.md).
...
When the code is written, run the task's **"Commands to run" verbatim** and show the real output. If a command fails, investigate whether the application code is wrong before touching any test (AGENTS.md testing rules). No completion claims without command output in hand.

...
On any failure, AGENTS.md testing rules apply: first investigate whether the application code is wrong; never weaken, delete, or rewrite tests just to make them pass; explain the root cause before changing production code or tests.

### 3. Spot-check touched invariants

Spot-check every domain invariant the task touches (AGENTS.md "Domain model & invariants") — especially:
